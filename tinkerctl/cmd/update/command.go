...
func run(ctx context.Context, args []string) error {
	...
	case resource.Post:
		...
		if post, err = c.Blog().UpdatePost(post); err != nil {
			return err
		}

		fmt.Printf("post %q was updated!\n", res.Name)
	...
	case resource.User:
		...
		user, err = c.Users().UpdateUser(user)
		if err != nil {
			return err
		}

		fmt.Printf("user %q was updated!\n", res.Name)
...
var (
	Info = &cmd.Info{
		Use:   "update",
		Short: "update a resource on the server",
		Long:  "update a resource on the server",
